...
describe('多板块导航与状态', () => {
  ...
  it('完整展示升级结果和经验表入口', () => {
    ...
    const currentLevel = screen.getByRole('spinbutton', { name: '当前等级' })
    const targetLevel = screen.getByRole('spinbutton', { name: '目标等级' })
    expect(currentLevel).toHaveValue(140)
    expect(targetLevel).toHaveValue(150)
    fireEvent.change(currentLevel, { target: { value: '141' } })
    fireEvent.click(screen.getByRole('button', { name: '选择目标等级' }))
    fireEvent.click(screen.getByRole('option', { name: '151 级' }))
    expect(currentLevel).toHaveValue(141)
    expect(targetLevel).toHaveValue(151)
    expect(screen.getAllByText('精英 AB + 修罗').length).toBeGreaterThan(0)
    expect(screen.getByText('精英 A + 修罗')).toBeVisible()
